...
"""
Script to record demonstrations with Isaac Lab environments using XR controller teleoperation for G1 humanoid robot.

This script includes Mink IK state synchronization for improved tracking consistency with XR controller
teleoperation on the Unitree G1 robot. It prevents drift between the Mink IK solver internal state and
the actual Isaac Lab simulation state.

This script allows users to record demonstrations operated by XR controller teleoperation for G1-based tasks.
...
required arguments:
    --task                    Name of the task (should be a G1-based task with XR support).
...
optional arguments:
    ...
    --step_hz                 Environment stepping rate in Hz. (default: 50, note: rate limiting is handled by OpenXR)
    --num_demos               Number of demonstrations to record. (default: 0)
    --num_success_steps       Number of continuous steps with task success for concluding a demo as successful. (default: 10)

Example usage:
    ./isaaclab.sh -p scripts/tools/record_demos_xr_g1.py --task Isaac-PickPlace-G1-InspireFTP-XR-v0
"""
...
parser = argparse.ArgumentParser(description="Record demonstrations for G1 Isaac Lab environments with XR controller.")
parser.add_argument("--task", type=str, required=True, help="Name of the task (G1-based with XR support).")
...
def handle_reset(
    ...
    arm_joint_ids: torch.Tensor | None = None,
    ...
    Args:
        ...
        arm_joint_ids: Optional joint IDs for G1 arm state sync
    ...
    # Sync teleop device state after reset (for G1 Mink IK state sync)
            ...
            for retargeter in teleop_interface._retargeters:
                if hasattr(retargeter, 'reset'):
                    ...
                    if hasattr(retargeter, 'set_robot_state'):
                        ...
                        # G1 Mink IK retargeter - try joint_positions kwarg
                        try:
...
def run_simulation_loop(
    ...
    # Setup for G1 Mink IK state synchronization
    arm_joint_ids = None
    robot = None
    ...
    if "robot" in env.scene.keys():
        ...
        try:
            ...
            omni.log.info("G1 Mink IK state synchronization enabled for XR teleoperation")
            omni.log.info(f"  Tracking {len(G1_ARM_JOINT_NAMES)} arm joints")
        except Exception as e:
            omni.log.warn(f"Could not find G1 arm joints for state sync: {e}. State sync disabled.")
            arm_joint_ids = None
    ...
    if arm_joint_ids is not None and robot is not None:
        ...
        if hasattr(teleop_interface, 'set_measured_joint_positions'):
            teleop_interface.set_measured_joint_positions(measured_joint_pos)
            omni.log.info("Initial G1 device state synchronized with simulation")
        if hasattr(teleop_interface, '_retargeters'):
            for retargeter in teleop_interface._retargeters:
                if hasattr(retargeter, 'reset'):
                    # Check which reset signature the retargeter supports
    ...
    with contextlib.suppress(KeyboardInterrupt) and torch.inference_mode():
        while simulation_app.is_running():
            # Update device state with measured joint positions (for G1 Mink IK state sync)
            if arm_joint_ids is not None and robot is not None:
                measured_joint_pos = robot.data.joint_pos[0, arm_joint_ids]
                if hasattr(teleop_interface, 'set_measured_joint_positions'):
                    teleop_interface.set_measured_joint_positions(measured_joint_pos)
