...
T1_W_GRIPPER_CFG = ArticulationCfg(
    spawn=sim_utils.UsdFileCfg(
        usd_path="source/isaaclab_assets/isaaclab_assets/robots/USD/t1/t1_29dof_1014.usd",
        activate_contact_sensors=True,
        rigid_props=sim_utils.RigidBodyPropertiesCfg(
            disable_gravity=False,
            retain_accelerations=False,
    ...
    soft_joint_pos_limit_factor=0.9,
    actuators={
        # Head: Kp=20, Kd=0.5 (matches TWIST)
        "head": ImplicitActuatorCfg(
            effort_limit_sim=7,
            velocity_limit_sim=12.56,
            joint_names_expr=["Head_pitch", "AAHead_yaw"],
            stiffness=20.0,
            damping=0.5,  # TWIST: 0.5 (was 0.2)
            armature=0.01,
        ),
        # Legs (Hip, Knee, Waist): Kp=150, Kd=5.0 (matches TWIST)
        "legs": DelayedPDActuatorCfg(
            joint_names_expr=[
                ".*_Hip_Yaw",
                ".*_Hip_Roll",
            ...
            stiffness=200.0,  # TWIST: 150 (was 200)
            damping=5.0,
            armature={
                ".*_Hip_.*": 0.01,
                ".*_Knee_Pitch": 0.01,
        ...
        # Feet (Ankles): Kp=10, Kd=1.0 (matches TWIST - compliant feet)
        "feet": DelayedPDActuatorCfg(
            joint_names_expr=[".*_Ankle_Pitch", ".*_Ankle_Roll"],
            effort_limit_sim={
                ".*_Ankle_Pitch": 24.0,
            ...
            stiffness=10.0,  # TWIST: 10 (was 50)
            damping=1.0,     # TWIST: 1.0 (was 3.0)
            armature=0.01,
            min_delay=10,
            max_delay=20,
        ),
        # Arms - Shoulder/Elbow: Kp=50, Kd=3.0 (matches TWIST)
        "arms_proximal": ImplicitActuatorCfg(
            ...
            stiffness=50.0,  # TWIST: 50
            damping=3.0,     # TWIST: 3.0
            armature={
                ".*_Shoulder_.*": 0.01,
                ".*_Elbow_.*": 0.01,
            },
        ),
        # Arms - Wrist/Hand: Kp=30, Kd=2.0 (matches TWIST)
        "arms_distal": ImplicitActuatorCfg(
            ...
            stiffness=30.0,  # TWIST: 30 (was 50)
            damping=2.0,     # TWIST: 2.0 (was 3.0)
            armature={
                ".*_Wrist_.*": 0.001,
                ".*_Hand_Roll": 0.001,
            },
...
"""Configuration for the Booster T1 humanoid robot with full body (29 DOF) and grippers (8 DOF, total 37 DOF).
PD gains match TWIST policy configuration for sim-to-real transfer."""